...
namespace TLS.Legacy.OAuth.Security.Token
{
    ...
    public class LegacyOAuthSecurityTokenHelper
    {
        ...
        public static AuthenticationTicket GetTicket(string token, LegacyTokenAuthenticationOptions options, bool ignoreRetry = false)
        {
            ...
            var raw = Unprotect(Base64UrlTextDecode(token), encryptionKey, validationKey, new CryptoAlgorithmFactory(options));

            if (raw == null && !ignoreRetry && token.Substring(0, 34).Where(m => m == '_').Count() == 2)
            {
                return GetTicket(token.Substring(34), options, true);
            }
        ...
        public static byte[] Base64UrlTextDecode(string token, bool ignoreRetry = false)
        {
            try
            {
                return Convert.FromBase64String(Pad(token.Replace('-', '+').Replace('_', '/')));
            }
            catch
            {
                if (!ignoreRetry && token.Substring(0, 34).Where(m => m == '_').Count() == 2)
                {
                    return Base64UrlTextDecode(token.Substring(34), true);
                }
                else
                {
                    throw;
