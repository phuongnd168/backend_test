...
namespace TLS.Lib.Job.Hangfire.Filters.Basic
{
    ...
    public class BasicAuthAuthorizationUser
    {
        ...
        /// <summary>
        /// If <see cref="Password"/> property is not set, this property will use to check user password to login
        /// This property value use base64 encode
        /// </summary>
        ...
        /// Password to check
        /// </summary>
        protected byte[] PasswordToCheck
        { 
            get
            {
                ...
                {
                    return Convert.FromBase64String(PasswordEncoded);
                }
                return null;
            }
        }
        ...
        public bool Validate(string login, string password, bool loginCaseSensitive)
        {
            ...
            if (login.Equals(Login, loginCaseSensitive ? StringComparison.InvariantCulture : StringComparison.InvariantCultureIgnoreCase) == true)
            {
                using (var cryptoProvider = CryptoProviderFactory())
                {
                    ...
                    return StructuralComparisons.StructuralEqualityComparer.Equals(passwordHash, PasswordToCheck);
                }
            }
            else
            {
